...
public class Tracking {
    ...
    public enum CurrentAnalisisStatus {

        REFRESH, REFRESH_LAST_CYCLE, RECYCLE, FINISHED
    };
        ...
        if (frame_2 == null) {
            ...
            for (sh = startIndex; sh < f.getShapes().size(); sh++) {
                ...
                if (s.getMatch() <= 0) {
                    ...
                    //TODO remove this from the for loop!
    ...
    /**
     * returns the shapes whom has the highest match level uses
     * compareWithTemplate()
     */
    ...
    /**
     * Warns the <code>Tracking</code> object that changes might have occured
     * and that it should check it's shape array.
     *
